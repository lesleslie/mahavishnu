...
def test_default_config_values():
    ...
    assert config.otel_storage_connection_string == ""
...
class TestOTelStorageConnectionStringSecurity:
    ...
    def test_empty_connection_string_when_disabled(self):
        ...
        config = MahavishnuSettings(
            ...
            otel_storage_connection_string="",
        )
        assert config.otel_storage_connection_string == ""
    ...
    def test_empty_connection_string_rejected_when_enabled(self):
        ...
        with pytest.raises(ValueError, match="otel_storage_connection_string must be set"):
            MahavishnuSettings(
                otel_storage_enabled=True,
                otel_storage_connection_string="",
            )
    ...
    def test_default_password_rejected(self):
        ...
        with pytest.raises(
            ...
            MahavishnuSettings(
                otel_storage_enabled=True,
                otel_storage_connection_string="postgresql://postgres:password@localhost:5432/otel_traces",
            )
    ...
    def test_postgres_postgres_rejected(self):
        ...
        with pytest.raises(
            ...
            MahavishnuSettings(
                otel_storage_enabled=True,
                otel_storage_connection_string="postgresql://postgres:postgres@localhost:5432/otel_traces",
            )
    ...
    def test_admin_admin_rejected(self):
        ...
        with pytest.raises(
            ...
            MahavishnuSettings(
                otel_storage_enabled=True,
                otel_storage_connection_string="postgresql://admin:admin@localhost:5432/otel_traces",
            )
    ...
    def test_root_root_rejected(self):
        ...
        with pytest.raises(
            ...
            MahavishnuSettings(
                otel_storage_enabled=True,
                otel_storage_connection_string="postgresql://root:root@localhost:5432/otel_traces",
            )
    ...
    def test_test_test_rejected(self):
        ...
        with pytest.raises(
            ...
            MahavishnuSettings(
                otel_storage_enabled=True,
                otel_storage_connection_string="postgresql://test:test@localhost:5432/otel_traces",
            )
    ...
    def test_invalid_scheme_rejected(self):
        ...
        with pytest.raises(
            ...
            MahavishnuSettings(
                otel_storage_enabled=True,
                otel_storage_connection_string="mysql://user:pass@localhost:3306/db",
            )
        ...
        with pytest.raises(
            ...
            MahavishnuSettings(
                otel_storage_enabled=True,
                otel_storage_connection_string="mongodb://user:pass@localhost:27017/db",
            )
    ...
    def test_postgres_scheme_accepted(self):
        ...
        config = MahavishnuSettings(
            otel_storage_enabled=True,
            otel_storage_connection_string="postgres://user:StrongPassword123!@localhost:5432/otel_traces",
        )
        assert config.otel_storage_connection_string == "postgres://user:StrongPassword123!@localhost:5432/otel_traces"
    ...
    def test_missing_at_separator_rejected(self):
        ...
        with pytest.raises(
            ...
            MahavishnuSettings(
                otel_storage_enabled=True,
                otel_storage_connection_string="postgresql://localhost:5432/otel_traces",
            )
    ...
    def test_missing_database_rejected(self):
        ...
        with pytest.raises(
            ...
            MahavishnuSettings(
                otel_storage_enabled=True,
                otel_storage_connection_string="postgresql://user:pass@localhost:5432",
            )
    ...
    def test_valid_connection_string_accepted(self):
        ...
        config = MahavishnuSettings(
            otel_storage_enabled=True,
            otel_storage_connection_string="postgresql://otel_user:xK9$mP2@nL5*qR7!localhost:5432/otel_traces",
        )
        assert (
            config.otel_storage_connection_string
    ...
    def test_valid_connection_string_with_port(self):
        ...
        config = MahavishnuSettings(
            otel_storage_enabled=True,
            otel_storage_connection_string="postgresql://otel_user:SecurePass456@db.example.com:5433/otel_traces",
        )
        assert config.otel_storage_connection_string == "postgresql://otel_user:SecurePass456@db.example.com:5433/otel_traces"

    def test_strong_password_with_special_characters(self):
        """Test that strong passwords with special characters are accepted."""
        config = MahavishnuSettings(
            otel_storage_enabled=True,
            otel_storage_connection_string="postgresql://user:P@ssw0rd!#$@localhost:5432/db",
        )
        assert "P@ssw0rd!#$@" in config.otel_storage_connection_string
    ...
    def test_password_not_containing_insecure_patterns(self):
        ...
        config = MahavishnuSettings(
            otel_storage_enabled=True,
            otel_storage_connection_string="postgresql://user:MypasswordIsVeryLong456!@localhost:5432/db",
        )
        # This should work because it's not exactly "password@"
        assert "MypasswordIsVeryLong456!@" in config.otel_storage_connection_string
    ...
    def test_connection_string_with_unix_socket(self):
        ...
        config = MahavishnuSettings(
            otel_storage_enabled=True,
            otel_storage_connection_string="postgresql://user:SecurePass@%2Fvar%2Frun%2Fpostgresql:5432/db",
        )
        assert config.otel_storage_connection_string == "postgresql://user:SecurePass@%2Fvar%2Frun%2Fpostgresql:5432/db"

    def test_connection_string_with_ssl_mode(self):
        """Test that connection strings with SSL parameters are accepted."""
        config = MahavishnuSettings(
            otel_storage_enabled=True,
            otel_storage_connection_string="postgresql://user:SecurePass123@localhost:5432/db?sslmode=require",
        )
        assert config.otel_storage_connection_string == "postgresql://user:SecurePass123@localhost:5432/db?sslmode=require"

    def test_environment_variable_override_description(self):
        """Test that error messages reference environment variable for setup."""
        with pytest.raises(
            ValueError, match="MAHAVISHNU_OTEL_STORAGE_CONNECTION_STRING"
        ):
            MahavishnuSettings(
                otel_storage_enabled=True,
                otel_storage_connection_string="",
        ...
        with pytest.raises(
            ...
            MahavishnuSettings(
                otel_storage_enabled=True,
                otel_storage_connection_string="postgresql://postgres:password@localhost:5432/db",
            )
