...
def main() -> None:
    ...
    for name in ["workflow_sweep", "workflow_quality_check", "workflow_heal", "workflow_fix", "workflow_review"]:
        if name in funcs:
            print(f"  ✅ {name}")
        else:
    ...
    print("\n=== Async helpers ===")
    for name in [
        "_async_trigger_workflow", "_async_heal_workflows", "_async_fix_orchestrate",
        "_async_review_and_fix", "_async_adapter_list", "_async_adapter_resolve",
    ...
    check("from .core.fix_orchestrator import FixOrchestrator, FixTask" in source, "FixOrchestrator import")
    check("from .core.task_router import TaskRouter" in source, "TaskRouter import")
    check("from .core.adapter_registry import HybridAdapterRegistry" in source, "HybridAdapterRegistry import")
    check("from .mcp.tools.self_improvement_tools import ReviewScope, SelfImprovementTools" in source,
          "SelfImprovementTools import")

    print("\n=== Correct API usage ===")
    check("dlq.retry_task" in source, "Heal uses DLQ retry_task")
    check("execute_fix(pool_id=pool_id, task=task)" in source, "FixOrchestrator call without timeout")
    ...
    check("await registry.check_all_health()" in source, "Adapter health uses async check_all_health")
