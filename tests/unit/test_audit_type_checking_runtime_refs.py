"""Tests for scripts/audit_type_checking_runtime_refs.py.
...
from scripts.audit_type_checking_runtime_refs import (
...
def test_cluster_by_import_site_groups_same_root_cause():
    ...
    from scripts.audit_type_checking_runtime_refs import FileResult
    results = [
...
def test_cli_module_runs(tmp_path):
    ...
    result = subprocess.run(
        [sys.executable, "scripts/audit_type_checking_runtime_refs.py", str(tmp_path)],
        capture_output=True,
        text=True,
    )
