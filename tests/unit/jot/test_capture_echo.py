...
def test_format_echo_simple() -> None:
    ...
    short_id is the LAST 6 hex chars of the UUID v4 ID (UD5).
    """
    result = format_echo("a3f9c2b1e8d74f6a9c1b2e3f4a5b6c7d", "hello world")
    assert result == "jot 5b6c7d captured (2 words, 11 chars)"


def test_format_echo_single_word() -> None:
    result = format_echo("abcdef0123456789" * 2, "test")
    assert result == "jot 456789 captured (1 words, 4 chars)"
...
def test_format_echo_uses_short_id_last_6() -> None:
    """Only the last 6 chars of the event ID appear in the echo (UD5)."""
    event_id = "fedcba9876543210fedcba9876543210"
    result = format_echo(event_id, "test")
    assert "543210" in result
