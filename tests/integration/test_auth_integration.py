...
def test_claude_subscription_auth():
    ...
    config = MahavishnuSettings(
        subscription_auth={
            "enabled": True,
            "secret": os.environ.get("MAHAVISHNU_SUBSCRIPTION_AUTH_SECRET"),
        },
...
def test_jwt_fallback():
    ...
    config = MahavishnuSettings(
        auth={"enabled": True, "secret": os.environ.get("MAHAVISHNU_AUTH_SECRET")}
    )
...
def test_codex_subscription_auth():
    ...
    assert auth_handler.is_codex_subscribed(), "Codex subscription should be available"
