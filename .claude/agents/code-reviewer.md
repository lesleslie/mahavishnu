...
description: >-
  ...
  Proactively reviews code after writing or modifying.
...
2. Call `mcp__akosha__search_code_patterns` for cross-repo pattern context
3. Review manually against checklist above
4. Report findings grouped by severity: **blocking** (gate failures) → **warnings** → **suggestions**
5. A review is complete only when `crackerjack run` reports zero failures
