...
class DatabaseRollback:
    ...
    def rollback_migration(self, target_version: str):
        ...
        # Get current version
        self.cursor.execute("SELECT version FROM schema_migrations ORDER BY version DESC LIMIT 1")
    ...
    def verify_metrics(self, checks: List[MetricCheck]) -> bool:
        ...
        for check in checks:
            try:
                ...
                print(f"{status} {check.name}: {value} {check.comparison} {check.threshold}")
...
if __name__ == "__main__":
    ...
    health_checks = [
        ...
        HealthCheck("Database Connection", "https://api.prod.example.com/health/db", 200),
        HealthCheck("Cache Connection", "https://api.prod.example.com/health/cache", 200),
    ]

    # Metric checks
    metric_checks = [
        MetricCheck("Error Rate", 'sum(rate(http_requests_total{status=~"5.."}[5m]))', 0.01, "lt"),
        MetricCheck("P95 Latency", 'histogram_quantile(0.95, http_request_duration_seconds_bucket)', 0.2, "lt"),
        MetricCheck("Active Pods", 'count(kube_pod_status_phase{phase="Running"})', 10, "gt"),
    ]
