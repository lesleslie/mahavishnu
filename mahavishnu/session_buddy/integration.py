...
from mcp_common.messaging.types import ProjectMessage, Priority
...
class SessionBuddyIntegration:
    ...
    def _extract_docstring_from_file(self, file_path: str, function_name: str) -> Optional[str]:
        ...
        try:
            ...
            for node in __import__('ast').walk(tree):
                if isinstance(node, (__import__('ast').FunctionDef, __import__('ast').AsyncFunctionDef, __import__('ast').ClassDef)):
    ...
    async def _index_documentation_in_session_buddy(self, repo_path: str, documentation: List[Dict[str, Any]]):
        ...
        try:
            ...
            self.logger.info(f"Indexing {len(documentation)} documentation items for {repo_path} in Session Buddy")
    ...
    async def send_project_message(self, from_project: str, to_project: str, subject: str, message: str, priority: Priority = Priority.NORMAL) -> Dict[str, Any]:
...
class SessionBuddyManager:
    ...
    async def process_repository_for_session_buddy(self, repo_path: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "overall_status": "success" if code_graph_result["status"] == "success" and doc_result["status"] == "success" else "partial"
        }
    
    async def get_enhanced_context(self, repo_path: str, query_elements: Dict[str, Any]) -> Dict[str, Any]:
