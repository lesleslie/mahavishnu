...
def evaluate_static(
    worker_type: str, *, config: WorkerConfig, ctx: StaticContext
    ...
    if config.requires_tool and shutil.which(config.requires_tool) is None:
        ...
        checks.append(WorkerCheck("binary", "fail", config.requires_tool))
    elif config.requires_tool:
        checks.append(WorkerCheck("binary", "pass", config.requires_tool))
    env_missing = [n for n in config.required_env if not ctx.env.get(n)]
    if env_missing:
        missing.extend(env_missing)
        checks.append(WorkerCheck("env", "fail", ",".join(env_missing)))
    elif config.required_env:
        checks.append(WorkerCheck("env", "pass", ",".join(config.required_env)))
    for key in config.required_settings:
        if not _resolve(ctx.settings, key):
            missing.append(f"setting:{key}")
            checks.append(WorkerCheck("setting", "fail", key))
