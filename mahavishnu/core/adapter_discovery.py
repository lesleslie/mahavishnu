...
2. **MCP MCP** - Remote adapter discovery via MCP's adapter registry tools
...
Example usage:
    ...
    mcp_adapters = await engine.discover_from_mcp()
"""
...
class AdapterMetadata:
    ...
    discovered from either entry points or MCP.
    ...
    Attributes:
        ...
        source: Discovery source ("entry_point" or "mcp")
    ...
    def from_adapter_entry(cls, entry: Any) -> AdapterMetadata:
        ...
        Args:
            entry: AdapterEntry from the MCP registry client
...
class AdapterDiscoveryEngine:
    ...
    2. MCP MCP adapter registry (if enabled)
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        Args:
            config: Optional configuration dictionary with keys:
                ...
                - mcp_registry_config: MCPAdapterRegistryConfig or dict
                - enable_entry_points: Enable entry point discovery (default: True)
                - enable_mcp_registry: Enable MCP registry discovery (default: True)
        """
        config = config or {}
        ...
        self._enable_mcp_registry: bool = config.get(
            "enable_mcp_registry",
            config.get("enable_oneiric_mcp", True),
        )

        # MCP adapter registry client configuration. Accept the legacy key
        # while callers migrate to mcp_registry_config.
        mcp_config = config.get("mcp_registry_config", config.get("oneiric_mcp_config"))
        if mcp_config and isinstance(mcp_config, dict):
            self._mcp_registry_config = MCPAdapterRegistryConfig(**mcp_config)
        elif mcp_config and isinstance(mcp_config, MCPAdapterRegistryConfig):
            self._mcp_registry_config = mcp_config
        else:
            self._mcp_registry_config = MCPAdapterRegistryConfig()
        ...
        # MCP registry client (lazy initialization)
        self._mcp_client: MCPAdapterRegistryClient | None = None

        logger.info(
            f"AdapterDiscoveryEngine initialized "
            f"(entry_points={self._enable_entry_points}, "
            f"mcp_registry={self._enable_mcp_registry}, "
            f"cache_ttl={self._cache_ttl_seconds}s, "
            f"allowlist_patterns={len(self._allowlist_patterns)})"
        )

    def _get_mcp_client(self) -> MCPAdapterRegistryClient | None:
        """Get or create MCP registry client (lazy initialization).

        Returns:
            MCPAdapterRegistryClient instance or None if unavailable
        """
        if not self._enable_mcp_registry:
            return None

        if self._mcp_client is None:
            try:
                self._mcp_client = MCPAdapterRegistryClient(self._mcp_registry_config)
                logger.info("MCP registry client initialized for adapter discovery")
            except Exception as e:  # noqa: BLE001 - CLI entrypoint; converts unhandled errors to exit
                logger.warning(f"Failed to initialize MCP registry client: {e}")
                return None
    ...
    async def discover_all(self) -> list[AdapterMetadata]:
        ...
        Combines results from entry points and MCP, deduplicating
        ...
        # MCP registry (fills in gaps not covered by entry points)
        if self._enable_mcp_registry:
            try:
                mcp_adapters = await self.discover_from_mcp()
                for adapter in mcp_adapters:
                    # Only add if not already discovered via entry points
                    if adapter.adapter_id not in all_adapters:
                        all_adapters[adapter.adapter_id] = adapter
            except Exception as e:  # noqa: BLE001 - boundary handler catches all errors to keep calling code alive
                logger.warning(f"MCP registry discovery failed (graceful fallback): {e}")
    ...
    async def discover_from_mcp(self) -> list[AdapterMetadata]:
        """Discover adapters from MCP's MCP adapter registry.

        Uses MCP adapter registry MCP tools to fetch adapter metadata
        from the remote registry.

        Returns:
            List of adapter metadata from MCP

        Note:
            Returns empty list if MCP is unavailable (graceful fallback)
        """
        cache_key = "mcp"
        ...
        if client is None:
            logger.debug("MCP registry client not available, returning empty list")
            return adapters

        try:
            # List all adapters from MCP registry
            ...
            for entry in entries:
                ...
                adapters.append(adapter_meta)
                logger.debug(f"Discovered adapter from MCP: {adapter_meta.adapter_id}")

        except ConnectionError as e:
            logger.warning(f"MCP registry connection error (graceful fallback): {e}")
            # Return empty list - graceful fallback
        except Exception as e:  # noqa: BLE001 - boundary handler catches all errors to keep calling code alive
            logger.error(f"MCP registry discovery failed: {e}")
        ...
        logger.info(f"Discovered {len(adapters)} adapters from MCP")
        return adapters
    ...
    def invalidate_cache_for_source(self, source: str) -> None:
        ...
        Args:
            source: Cache key/source to invalidate ("entry_points", "mcp", etc.)
        """
        with self._lock:
    ...
    async def close(self) -> None:
        ...
        Closes the MCP registry client if initialized.
        """
        with self._lock:
            if self._mcp_client is not None:
                try:
                    await self._mcp_client.close()
                    logger.info("MCP registry client closed")
                except Exception as e:  # noqa: BLE001 - boundary handler catches all errors to keep calling code alive
                    logger.warning(f"Error closing MCP registry client: {e}")
                finally:
