...
class MahavishnuError(Exception):
    ...
    RECOVERY_GUIDANCE: ClassVar[dict[str, list[str]]] = {
        ...
        ErrorCode.WORKTREE_LOCKED: [
            "The worktree registry row is locked by another operation",
            "Wait for the in-flight operation to complete and retry",
            "Use 'mahavishnu worktree list' to inspect lock holders",
            "If the lock is stuck, restart the orchestrator process to clear it",
        ],
        ErrorCode.WORKTREE_INTEGRITY_FAILED: [
            "Worktree integrity verification failed",
            "Re-run 'mahavishnu worktree verify <handle-id>' to reproduce",
            "Compare on-disk sha256 against the stored handle sha256",
            "If the storage backend corrupted, restore from backup",
        ],
        ErrorCode.WORKTREE_BUNDLE_TEMP_CREATE_FAILED: [
            "Could not create a temp file for the worktree bundle",
            "Check disk space on the bundle staging filesystem",
            "Verify OS-level mkstemp permissions on the temp directory",
            "Re-run the workflow; transient tmpfs/SSD pressure can cause this",
        ],
        ErrorCode.WORKTREE_BUNDLE_TEMP_WRITE_FAILED: [
            "Writing the bundle to temp failed (OS error or cancellation)",
            "Check disk space on the bundle staging filesystem",
            "Confirm no CancelledError interrupted the write",
            "Re-run the workflow; if it persists, capture the OS error code",
        ],
        ErrorCode.WORKTREE_BUNDLE_PATH_TRAVERSAL: [
            "Bundle contained a path-traversal entry rejected by the data filter",
            "Inspect the tarball member list with 'tar -tzf' to find the entry",
            "Reject any bundle that mixes absolute or parent-directory paths",
            "Re-export the source worktree with a hardened archive command",
        ],
        ErrorCode.WORKTREE_BUNDLE_MALFORMED: [
            "Worktree bundle is corrupt, truncated, or not in the expected codec",
            "Re-export the source worktree with 'mahavishnu worktree bundle'",
            "Verify the storage backend checksum against the stored sha256",
            "If the storage backend corrupted, restore from backup",
        ],
        ErrorCode.WORKTREE_BUNDLE_LEGACY_PHASE2: [
            "Fetched a Phase 2 .tar.gz bundle — only Phase 3 zstd bundles are supported",
            "Re-export the source worktree with 'mahavishnu worktree bundle' to get a zstd bundle",
            "Decompress the legacy .tar.gz manually if you must recover the data",
            "Delete the legacy bundle from storage once recovered",
        ],
        ErrorCode.WORKTREE_BUNDLE_STORAGE_KEY_TOO_LONG: [
            "Storage key exceeds the backend's 1024-byte limit (S3)",
            "Shorten repo path or remove deep nested directories from the bundle",
            "Use a storage backend with longer key support (GCS/Azure)",
            "Re-export the worktree after restructuring paths",
        ],
        ErrorCode.WORKTREE_BUNDLE_STOPGAP_TOO_LARGE: [
            "Worktree bundle exceeds the in-memory stopgap OOM guard",
            "Increase the bundle size threshold in settings/mahavishnu.yaml",
            "Split the worktree into smaller bundles via 'mahavishnu worktree bundle --shard'",
            "Use a streaming-aware downstream loader instead of full materialization",
        ],
        ErrorCode.WORKTREE_BUNDLE_NOT_FOUND: [
            "Storage adapter returned None for the requested bundle key",
            "Confirm the bundle exists in the configured storage backend",
            "Check storage backend credentials and bucket/container name",
            "Re-export the source worktree if the bundle was lost",
        ],
        ErrorCode.WORKTREE_BUNDLE_CODEC_UNAVAILABLE: [
            "zstandard codec is not installed — required for Phase 3 bundle format",
            "Install with 'uv pip install zstandard' (or 'pip install zstandard')",
            "Re-export the bundle with 'mahavishnu worktree bundle' once installed",
            "Fall back to a Phase 2 .tar.gz bundle if zstandard cannot be installed",
        ],
        ...
        ErrorCode.PRECOMMITMENT_SIGNATURE_MISMATCH: [
            ...
            "Verify the MCPLock backend has not been tampered with",
