...
``WebSocketServer`` exposes ``set_event_publisher()`` for runtime
...
- ``enabled=True`` AND ``dry_run=False`` AND ``bridge`` provided ->
  ...
  ``server.set_event_publisher(...)`` with it, and returns the new
  publisher.
...
class _WebSocketServerProtocol(Protocol):
    def set_event_publisher(self, publisher: Any | None) -> None: ...
...
def resolve_event_publisher(
    ...
    *,
    ...
    Args:
        ...
        server: WebSocket server instance with a ``set_event_publisher``
            method (production injects via this).
    ...
    server.set_event_publisher(publisher)
    return publisher
