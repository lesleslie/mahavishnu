...
class ErrorRecoveryManager:
    ...
    async def classify_error(self, error: Exception) -> ErrorCategory:
        ...
        if any(keyword in error_str for keyword in [
            "memory", "disk", "quota", "capacity",
            "resource", "oom", "out of memory", "out of",
            "exceeded", "space", "device"
        ]) and not any(exclude in error_str for exclude in [
            "index", "range"  # Exclude index/range errors which are usually permanent
        ]):
        ...
        if any(keyword in error_str for keyword in [
            "invalid", "malformed", "validation", "format",
            "valueerror", "typeerror", "schema", "assertion", "assertionerror"
        ]):
    ...
    async def _skip_and_continue(
        ...
        self.logger.warning(f"Skipping operation for workflow {workflow_id} due to error: {str(error)}")
        ...
        if self.app.observability:
            if success:
                self.app.observability.log_info(
                    f"Operation completed for workflow {workflow_id}",
                    attributes=log_entry
                )
            else:
                self.app.observability.log_error(
                    f"Operation failed for workflow {workflow_id}: {error}",
                    attributes=log_entry
                )
    ...
    async def _log_recovery_action(
        ...
        if self.app.observability:
            self.app.observability.log_info(
                f"Recovery action '{action}' for workflow {workflow_id}",
                attributes=log_entry
            )
    ...
    async def monitor_and_heal_workflows(self):
        ...
        try:
            ...
            self.logger.info(f"Healed {healed_count} workflows out of {len(failed_workflows)} failed workflows")
    ...
    async def _heal_workflow(self, workflow_id: str, workflow_state: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            adapter_name = original_task.get("adapter") or workflow_state.get("adapter", "llamaindex")
                    ...
                    try:
                        ...
                        if current_time - updated_at > timeout_threshold:
                            ...
                            if workflow_id:
                                ...
                                self.logger.warning(f"Marked stuck workflow {workflow_id} as failed")
                    except ValueError:
