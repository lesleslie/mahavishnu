...
try:
    ...
    class MockCounter:
        def add(self, amount: int, attributes: Dict[str, str] = None): pass
    
    class MockHistogram:
        def record(self, amount: float, attributes: Dict[str, str] = None): pass
    ...
    class MockTracer:
        def start_as_current_span(self, name: str, attributes: Dict[str, str] = None):
            class MockSpan:
                ...
                def set_attribute(self, key: str, value: str): pass
            return MockSpan()
    
    class MockMeter:
        def create_counter(self, name: str) -> MockCounter: return MockCounter()
        def create_histogram(self, name: str) -> MockHistogram: return MockHistogram()
        def create_up_down_counter(self, name: str) -> MockUpDownCounter: return MockUpDownCounter()
    
    class MockTraceProvider:
        def get_tracer(self, name: str) -> MockTracer: return MockTracer()
    
    class MockMeterProvider:
        def get_meter(self, name: str) -> MockMeter: return MockMeter()
...
class ObservabilityManager:
    ...
    def _init_fallback_components(self):
        ...
        self.workflow_duration_histogram = self.meter.create_histogram("mahavishnu.workflow.duration")
        self.repo_processing_duration_histogram = self.meter.create_histogram("mahavishnu.repo.processing.duration")
    ...
    def log(self, level: LogLevel, message: str, attributes: Dict[str, Any] = None, trace_id: str = None):
        ...
        getattr(self.logger, level.value.lower())(f"[{trace_id}] {message}" if trace_id else message)
    ...
    def get_logs(self, limit: int = 100, level: LogLevel = None, since: datetime = None) -> List[LogEntry]:
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_errors": len([log for log in self.logs[-50:] if log.level in [LogLevel.ERROR, LogLevel.CRITICAL]])
        }
