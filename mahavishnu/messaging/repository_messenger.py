...
class RepositoryMessenger:
    ...
    def __init__(self, app):
        ...
        self.authenticator: Optional[CrossProjectAuth] = None
    ...
    def unsubscribe(self, repo_name: str, callback: callable):
        ...
        if repo_name in self.subscribers:
            if callback in self.subscribers[repo_name]:
    ...
    async def send_message(
        ...
        try:
            ...
            self.logger.info(f"Message sent from {sender_repo} to {receiver_repo}: {message_type.value}")
    ...
    async def broadcast_message(
        ...
        try:
            ...
            self.logger.info(f"Broadcast message from {sender_repo} to {len(sent_messages)} repositories")
...
class RepositoryMessengerManager:
    ...
    async def process_repository_changes(self, repo_path: str, changes: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def send_quality_alert(
        ...
        try:
            ...
            messages = await self.messenger.broadcast_message(
                ...
                content=content,
                priority=MessagePriority.HIGH if severity.lower() in ["high", "critical"] else MessagePriority.NORMAL
            )
