...
The previous version of this module carried a 21-field model with
...
Note: the previous local schema did NOT enforce the B-6 body-integrity
``model_validator`` (``sha256(system_prompt) == content_hash``) — that
check is now part of the canonical schema and applies to every
component's AgentMetadata (the agents_tools layer in mahavishnu still
asserts the hash at sign time).
