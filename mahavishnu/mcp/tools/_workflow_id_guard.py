...
before touching MCP. On mismatch, callers should return
``{"workflow_id": workflow_id, "status": "invalid_workflow_id"}`` so MCP
callers see a consistent error shape and MCP is never touched with a
tainted key.
...
the value is spliced into a MCP key path, otherwise a caller can read
...
def validate_workflow_id(workflow_id: str) -> bool:
    """Return True iff ``workflow_id`` is a safe MCP key fragment."""
...
def validate_approval_id(approval_id: str) -> bool:
    ...
    value reaches the MCP key path.
    """
