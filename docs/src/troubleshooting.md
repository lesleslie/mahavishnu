...
2. Ensure authentication is properly configured in settings
3. Check that JWT tokens are not expired
...
2. Verify adapter-specific configuration
...
2. Ensure paths are absolute or properly relative
...
2. Check the configured port is available
...
2. Increase `timeout_per_repo` if processing large repositories
...
2. Wait for the circuit breaker timeout to reset
...
6. Full error message and traceback
7. Log snippets showing the issue
