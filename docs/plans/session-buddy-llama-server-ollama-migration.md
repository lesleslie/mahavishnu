...
2. `OLLAMA_URL` (default: `http://localhost:11434`)
3. ONNX (if available, transition fallback only)
4. Return `None`
...
2. ✅ `health_checks.py` fixed: llama-server URL uses same `_get_llama_server_url()` stripping logic to avoid double `/embeddings` (line 257)
3. ✅ `generate_embedding("test")` returns 384d `list[float]` via HTTP path when servers are up; returns `None` when servers are down (correct graceful degradation)
4. ✅ Thread-safe dict cache replaces broken `@lru_cache` — stores results, evicts oldest 10% when >1024 entries
5. ✅ `SkillsEmbeddingService.initialize()` fixed: no longer checks `session is not None` (which was always `None` since `initialize_embedding_system()` is now a no-op)
6. ✅ `knowledge_graph_adapter_oneiric.py` fixed: removed `self._embedding_session is None` guard that was always true (line 481)
7. ✅ `reflection_adapter_oneiric.py`: `_generate_embedding()` correctly `await`s `generate_embedding()` with no `run_until_complete`
8. ✅ All imports clean across `reflection_adapter_oneiric`, `knowledge_graph_adapter_oneiric`, `storage/skills_embeddings`
9. ⏳ All tests pass: `pytest tests/ -m "not slow"` (known-broken tests excluded per prior review)

### ✅ Crackerjack (Phase 4 complete)
1. ✅ `uv sync` resolves cleanly — 288 packages, no sentence-transformers/transformers conflicts
2. ✅ `get_issue_embedder()` returns `FallbackIssueEmbedder` (TF-IDF) when sentence-transformers unavailable — no code changes needed
3. ✅ Semantic search degrades gracefully via TF-IDF fallback (100d) — acceptable for QC use case
4. ⏳ All tests pass: `pytest tests/ -m "not slow"`

### ✅ Ecosystem-wide
1. ✅ No Bodai component transitively pulls in `onnxruntime`, `transformers`, or `sentence-transformers`
2. ✅ All embedding requests route through llama-server → Ollama → degraded mode
3. ✅ `uv sync` clean across session-buddy (286), crackerjack (288), mahavishnu ecosystem
