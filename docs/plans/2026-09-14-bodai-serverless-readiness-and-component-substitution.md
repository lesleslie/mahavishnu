...
| REQ-HARNESS-ACP-INTEGRATION | **Consolidated with active plan `docs/plans/2026-07-26-mahavishnu-acp-server.md`** (status: active — frontmatter flipped 2026-09-19 per operator signoff; this REQ's claim that the referenced plan is active is now consistent across both files). That plan already builds Mahavishnu as an ACP server (`mahavishnu/acp/`, `mahavishnu acp serve` CLI, stdio JSON-RPC 2.0). Phase 11 validates Qwen Code can drive Mahavishnu through the existing ACP server surface via any ACP client (Zed, JetBrains, Toad). **No new ACP code in Phase 11**; this REQ is purely validation that the existing ACP server works for harness-agnostic use cases. **This REQ remains blocked on the ACP plan's completion** (Phases 1–4 shipped + Phase 4 e2e green); the 2026-09-19 activation is paperwork, not implementation. ACP itself is at https://github.com/agentclientprotocol/agent-client-protocol — Apache 2.0, 4.2k stars, maintained by Zed/JetBrains/OpenHands/GitHub/Pydantic. | Phase 11 |
