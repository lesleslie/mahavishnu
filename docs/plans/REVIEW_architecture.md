...
2. **Rolling window boundary races**: The analyzer queries traces within `window_minutes` (default 60s). At t=0 and t=60s, different time windows produce different trace sets. A selector's score can oscillate wildly if the boundary falls across a gap in trace collection. The plan acknowledges 1 hour as the default (line 213) but doesn't address window alignment.
...
2. Scalability: single Akosha analyzer cannot handle 100+ repos' trace volume
3. Push model contradicts the standalone constraint — the feedback loop requires Mahavishnu to be running