...
2. Akosha analyzes routing outcomes to compute per-selector fitness signals
3. Mahavishnu's pool manager uses those signals to pick the best routing strategy
...
2. **Selector score formula**: Simple `1 - failure_rate` is a starting point. Could add latency
...
3. **Rolling window**: 1 hour is the starting default. Should be configurable per task_class.
   → **Start with**: global config, per-task override in Akosha's analyzer config.

4. **Akosha's Mahavishnu MCP client**: How does Akosha call `query_component_traces`?
