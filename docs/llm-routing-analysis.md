...
1. Model selection based on category
1. Fallback matching by model family
1. Final fallback to config default
...
1. **Phase 2:** Update Crackerjack adapters
1. **Phase 3:** Implement hybrid routing (local + gateway)
...
1. Test performance with existing model routing
1. Consider direct integration for specialized use cases
...
1. Update Session-Buddy to use gateway by default
1. Update Mahavishnu Agno adapter
...
1. **Performance** - Gateway should not introduce significant latency
1. **Reliability** - Fallback mechanisms for gateway failures
1. **Monitoring** - Clear visibility into gateway vs direct API usage
1. **Configuration** - Easy switching between gateway and direct modes
