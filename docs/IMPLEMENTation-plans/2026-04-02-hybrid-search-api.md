...
- **Key Components**:
  ...
  1. **HybridSearchResult** (Pydantic model)
...
- **Tools**:
  ...
  1. **index_document**: Index a document for search
     - Parameters: doc_id, title, content, metadata
     - Returns: Success status
  1. **delete_document**: Remove document from search index
     - Parameters: doc_id
     - Returns: Success status
  1. **search_by_repository**: Convenience tool for repository-scoped search
...
1. **Configurable Weights**: Adjustable semantic vs lexical weight balance (default 70/30)
1. **Graceful Fallback**: Falls back to lexical-only search when embeddings unavailable
1. **Repository Filtering**: Optional repository filter for all search operations
1. **Document Indexing**: Full document indexing with automatic embedding generation
1. **MCP Integration**: Exposes search functionality via MCP tools
1. **Observability**: Comprehensive logging for all operations
...
1. Performance optimization for query tuning
1. Add caching layer for frequently accessed documents
1. Implement reindexing for bulk updates
1. Add search analytics and usage metrics
