...
| File | Topic | Verified state (2026-09-10) |
|------|-------|------------------------------|
| `2026-09-10-tier2-optimal-transport.md` | Tier 2: optimal transport (Wasserstein) for Akosha cross-system pattern comparison. Re-eval trigger: `akosha.patterns.cross_system_snapshot_count ≥ 10,000` over 30-day window. | 🟡 **Deferred (active followup)** — Tier 1 Phase 9 (`scripts/feature_eligibility.py` + `make tier2-eligibility`) wires the trigger to monthly CI; plan opens when the trigger fires. |
| `2026-09-10-tier2-hyperbolic-embeddings.md` | Tier 2: hyperbolic embeddings (Poincaré disk) for Session-Buddy code graphs. Re-eval trigger: `session_buddy.code_graph.node_count ≥ 10,000` AND coverage `≥ 0.60`. | 🟡 **Deferred (active followup)** — same machinery as above. |
| `2026-09-10-tier2-phase-d-cross-repo.md` | Tier 2 / Phase D: cross-repo CUSUM/PH detector wiring into Akosha's `pytrendy` slot via `fitness_analyzer.py:279` task_classes extension. Re-eval trigger: Tier 1 Phase 8 `adopted` AND `≥ 1 week` of production operation. | 🟡 **Deferred (active followup, blocks-on: phase-8-adopted-and-stable)** — same machinery. |
| `2026-09-10-tier2-multi-metric-drift.md` | Tier 2: extend Tier 1's single-metric change-point detector to N metrics. Re-eval trigger: `≥ 3` distinct metrics requested by operators in 30-day window. | 🟡 **Deferred (active followup, operational follow-on trigger added in Tier 1 v3.1)** — same machinery. |
...
| `2026-07-15-sb-checkpoint-stash-clobber.md` | Recurring: auto-checkpoint hook re-applies a `git stash` over in-flight subagent edits. | 🟡 **Partially resolved (2026-09-06)** — the session-buddy `SubagentDetector` read-side is in place and wired into `policy.py` + `orchestrator.py`, but the **producer** (lockfile creation at subagent start) is owned by the subagent-runtime team and has not landed. The detector is *structurally* in place but *functionally* dormant: nothing creates the lockfile, so `is_active()` always returns False in current code. The remaining work is the producer half, tracked externally. |
