...
**Status:** Resolved (Option C, opt-in fail-closed). Added `dlq.fail_on_opensearch_unavailable` config flag (default `False` for back-compat); `DeadLetterQueue.__init__` accepts the same flag and `enqueue` raises `ExternalServiceError` when the flag is on and OpenSearch is unavailable. Covered by `tests/unit/core/test_dead_letter_queue_fail_closed.py` (4 tests, all green). Discovered during the bodai-crow-server runbook + `local.yaml.example` PR audit.
