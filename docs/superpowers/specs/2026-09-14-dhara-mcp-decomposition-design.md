...
8. **Replace Mahavishnu's `from akosha.storage import HotStore`** (resolves §4.12 cross-component Postgres coupling + ADR 017 §Implementation evidence line 147). The forward reference at `mahavishnu/ingesters/otel_ingester.py:41` (in `TYPE_CHECKING`) becomes `from oneiric.adapters.vector.hot_store import HotStore` — Oneiric's substrate-hosted Protocol. **Protocol lifted 2026-09-15 in oneiric commit `93f60cd`.** The runtime imports at lines 605 and 1334 (which construct the concrete `HotStore`) follow in a follow-up commit that ports the concrete class to `oneiric/adapters/vector/duckdb_hot_store.py`; until that lands, Mahavishnu's runtime path still points at `akosha.storage.HotStore` (AkoSHA → Oneiric dependency direction is fine per ADR 017, but Mahavishnu → AkoSHA is not). **Replacement path:** `from oneiric.adapters.vector.hot_store import HotStore` for the type annotation (done); `from oneiric.adapters.vector.duckdb_hot_store import DuckdbHotStore` for runtime construction (pending); **never** `from akosha.storage import HotStore` after the full concrete-lift commits. The dependency arrow flips: Mahavishnu → Oneiric, never Mahavishnu → AkoSHA. **Demonstrable by:** `grep -rn "from akosha" mahavishnu/` returns 0 matches after Phase 5 commits; the OTel ingester continues to read/write trace data via the Oneiric adapter; the e2e test from Phase 5 task 5 passes with the new adapter path. **Rollback signal:** trace ingestion returns 0 records, or `pytest mahavishnu/tests/integration/test_otel_ingester_e2e.py` fails.
