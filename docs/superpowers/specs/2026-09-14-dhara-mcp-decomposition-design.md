...
| Oneiric | `OTelStorageSettings.connection_string` default `"postgresql://postgres: postgres@localhost: 5432/otel"` has spaces around the colon (real bug at `oneiric/adapters/observability/settings.py:9`); Mahavishnu's validator (`mahavishnu/core/config.py:846-917` — `OTelIngesterConfig.validate_connection_string`) does NOT catch this — its `startswith/"/count/@-count` checks all pass with whitespace embedded (verified 2026-09-15; spec's prior claim of defense-in-depth was false). | Phase 10 fixes the default (literal edit at `oneiric/adapters/observability/settings.py:9`); optionally adds `.strip()` at the top of `validate_connection_string` to harden against future env-var whitespace leakage. **Single load-bearing safety net:** the Oneiric literal fix. Mahavishnu's validator provides no defense for this defect. |
...
- **Bus choice**: `oneiric.adapters.bootstrap.queued_publisher()` — the canonical factory entry point. Under the hood it returns `RedisStreamsQueueAdapter` from `oneiric.adapters.queue.redis_streams`. Lazy-loads the redis_streams adapter (per the brainstorming session 2026-09-14; aligned with the active serverless-readiness plan Phase 8 WAL substrate). The factory itself lives at `oneiric/adapters/bootstrap.py` (added 2026-09-15 to resolve Pass 2 finding W1; the plan's Phase 12a references this factory directly).
...
`~/.mahavishnu/bodai-event-queue.json` (current) → `oneiric.adapters.bootstrap.queued_publisher()` (target; returns `RedisStreamsQueueAdapter` from `oneiric.adapters.queue.redis_streams`). **Hard cutover in Phase 12a task 5** — all in-process Bodai producers either publish to the bus or are retired; the JSON-file path is deleted. No dual-write migration state (Phase 5 R11 cautionary tale: `persistence.write_mode: "dual"` is precisely the kind of debt this design rule rejects).
...
8. **Replace Mahavishnu's `from akosha.storage import HotStore`** (resolves §4.12 cross-component Postgres coupling + ADR 017 §Implementation evidence line 147). **Both layers lifted 2026-09-15.** The forward reference at `mahavishnu/ingesters/otel_ingester.py:41` (in `TYPE_CHECKING`) becomes `from oneiric.adapters.vector.hot_store import HotStore` — Oneiric's substrate-hosted Protocol (oneiric commit `93f60cd`). The runtime imports at lines 605 and 1334 (which construct the concrete `HotStore`) become `from oneiric.adapters.vector.duckdb_hot_store import DuckdbHotStore` — Oneiric's substrate-hosted concrete (oneiric commit `198564e`). **Replacement path:** `from oneiric.adapters.vector.hot_store import HotStore` for the type annotation; `from oneiric.adapters.vector.duckdb_hot_store import DuckdbHotStore` for runtime construction. **Never** `from akosha.storage import HotStore`. The dependency arrow flips: Mahavishnu → Oneiric, never Mahavishnu → AkoSHA. **Demonstrable by:** `grep -rn "^from akosha\|^import akosha" mahavishnu/` returns 0 matches; the OTel ingester continues to read/write trace data via the Oneiric adapter; the e2e test from Phase 5 task 5 passes with the new adapter path. **Rollback signal:** trace ingestion returns 0 records, or `pytest mahavishnu/tests/integration/test_otel_ingester_e2e.py` fails.
