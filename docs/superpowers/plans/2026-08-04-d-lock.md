...
## v1.1 Update (2026-08-10)

This is a minimal post-shipment update. Tasks 1-14 above have all shipped. The notes below document what landed, what changed in v1.1, and what remains deferred.

### Shipped artifacts (post-v1)

- `dhara/lock/` — Protocol (`DharaLock`), SQL impl (`SQLBackendLock`), REST routes under `/locks/`, observability events (`audit:lock.acquired` / `released` / `heartbeat` / `lost`), `InMemoryDharaLock` fixture, DuckDB-backed migration 0003 (`substrate_locks` table).
- `mahavishnu/cli/precommit_cli.py` — async `HypothesisLock` over `SQLBackendLock` (replaces `LockStore` + `JsonFileLockStore`). `LockStore` and `JsonFileLockStore` fully retired.
- `dhara/mcp/server_core.py` — `register_lock_routes` wired into `DharaMCPServer._register_tools()`.
- Cross-instance persistence verified via file-backed DuckDB (`tmp_path / "test.db"`).

### v1.1 changes

- **Consumer integration guidance** — added inline below for the 5 D-OBJ-SCHEMA entity wire-up plans that will land in parallel:
  - Each consumer's write path calls `validate("audit_record", payload)` etc. via `SCHEMA_REGISTRY` from `dhara.schema` (already shipped at `dhara/main` commit `e334425`).
  - Where durable writes happen, the producer side should call `dhara.put(...)` (which triggers the D-AUDIT subscriber once that substrate ships).
  - Lock contention is recorded as `audit_record` with `metadata={"lock_key": ..., "owner_token": ...}` — provides automatic audit-trail of all D-LOCK activity.

### Out-of-scope clarifications (still deferred)

- Postgres translation target — DuckDB → Postgres. Tracked as **post-v1.1 follow-up**.
- Cross-process lock clustering — multiple DharaMCPServer instances sharing one Postgres substrate. Tracked as post-v1.1 follow-up.
- Lock TTL extension policies beyond the current `extend_seconds=None` reset-to-original semantics.
