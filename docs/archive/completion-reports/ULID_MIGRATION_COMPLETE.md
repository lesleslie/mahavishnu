...
All three ecosystem systems (Crackerjack, Session-Buddy, Akosha) have been successfully migrated to use Dhara ULID identifiers. Migration was executed on fresh database installations with zero legacy data to migrate - ideal scenario for clean ULID adoption.
...
3. ✅ ULID migration
   ...
   - New jobs will use `from dhara import generate` on creation

**Next Steps** (Code Updates Required):
- Update `crackerjack/services/metrics.py`: Replace `uuid.uuid4()` with `from dhara import generate`
- Update test fixtures to use Dhara ULID format
...
**Next Steps** (Code Updates Required):
- Update `session_buddy/session_manager.py`: Replace `f"{project_name}-{timestamp}"` with `from dhara import generate`
...
**Next Steps** (Code Updates Required):
- Update `akosha/processing/knowledge_graph.py`: Replace `f"system:{id}"` and `f"user:{id}"` with `from dhara import generate`
- Update `akosha/mcp/tools/akosha_tools.py`: Replace custom ID generation with `generate()`
- Add imports: `from dhara import generate, ULID`
...
✅ **Dhara ULID**: Thread-safe monotonic randomness, 19,901 ops/sec
...
**Crackerjack**:
1. Update `crackerjack/services/metrics.py` line ~100: Replace `job_id = str(uuid.uuid4())` with `from dhara import generate; job_id = generate()`
2. Update test fixtures in `tests/` to use ULID format

**Session-Buddy**:
1. Update `session_buddy/session_manager.py`: Replace session_id generation with `from dhara import generate`
2. Update reflection creation to use ULID identifiers
3. Create database schema with session_ulid column

**Akosha**:
1. Update `akosha/processing/knowledge_graph.py`: Replace entity_id generation with `from dhara import generate`
2. Update `akosha/mcp/tools/akosha_tools.py`: Use Dhara for entity IDs
...
### Immediate Actions
1. ✅ Update application code to use Dhara `generate()` in all three systems
