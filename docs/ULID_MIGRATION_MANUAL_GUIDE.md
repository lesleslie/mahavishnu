...
1. **Initialize Session-Buddy Database**:
...
1. **Run Crackerjack ULID Migration**:
...
1. **Run Session-Buddy ULID Migration**:
...
1. **`migrate_session_buddy_to_ulid.py`** - Session-Buddy session ULID migration
...
1. **`migrate_akosha_to_ulid.py`** - Akosha entity ULID migration

   - Supports `--dry-run` for safe preview
   - Updates GraphEntity class code
   - No database changes (in-memory)

1. **`migrate_all_systems_to_ulid.sh`** - Master migration orchestrator
...
1. **Issue**: Empty databases (no tables)
...
1. Run individual migration scripts (or use master orchestrator)
1. Verify migrations with validation SQL queries
1. Run cross-system integration tests
